...
def q_t(t):                                             # Define como se comporta q_t durante el tiempo (Entrega J/m³)
    tiempo = t/3600.                                    # Transforma el tiempo en segundos a horas
    if tiempo < 1.5:                                    #En la primera hora y media no existe reaccíon exotermica
        return 0                               
    ...
    elif -0.1234*t + 7.7239 >= 0:                       # Verifica de que q(t) no sea negativo (Esto es dado que la ecuación es lineal y en algún momento se torna negativa)
...
# Parametros del aire para calcular condición de borde al exterior
K_aire = 0.02
c_aire = 1.012
rho_aire = 1225
...
n_max=24*60*60*14   # Tiempo a analizar en s [14 dias]
...
for n in range(n_max):
    ...
    for i in range(1,Nx): 
    	for j in range(1,Ny): 
            for k in range(0,Nz):
    	        ...
    	        #Laplaciano
                if k == 0:      #se calcula la temperatura del borde expuesto utilizando un alfa que es la raón de conduccion de temperatura en el aire
    	        ...
    	        #Forward euler
    	        u_nm1[i,j,k] = u_n[i,j,k] + alpha*nabla_u_n + q_t(n)/(c*rho) #q(t) entrega J/m³ por lo que se divide en c y rho para transofrmarlo en temperatura
    ...
    u_n = u_nm1
    if n % 3600 == 0: #ingresa cada 3600 segundos para plotear el estado del hormigón
